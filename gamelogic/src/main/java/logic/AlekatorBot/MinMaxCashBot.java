...
/**
 * Реализация бота с использованием алгоритма Minimax с мемоизацией.
 */
public class MinMaxCashBot extends Player {
    ...
    /**
     * Создает нового бота MinMaxCashBot.
     *
     * @param playerCell      Символ игрока (черные или белые).
     * @param maxDepth        Максимальная глубина поиска в алгоритме Minimax.
     * @param timeLimitMillis Ограничение по времени на ход бота в миллисекундах.
     */
    private Map<String, Integer> positionCache = new HashMap<>();

    public MinMaxCashBot(Cell playerCell, int maxDepth, long timeLimitMillis) {
        super(playerCell);
    ...
    private MinimaxResult minimax(Board board, int depth, int alpha, int beta, boolean maximizingPlayer, long endTime) {
        ...
        if (positionCache.containsKey(positionKey)) {
            int cachedScore = positionCache.get(positionKey);
            return new MinimaxResult(null, cachedScore);
        }

        if (depth == 0 || availableMoves.isEmpty() || board.isGameOver() || System.currentTimeMillis() >= endTime) {
            int score = evaluate(board);

            positionCache.put(positionKey, score);
        ...
        if (maximizingPlayer) {
            ...
            positionCache.put(positionKey, maxScore);
            ...
            positionCache.put(positionKey, minScore);
